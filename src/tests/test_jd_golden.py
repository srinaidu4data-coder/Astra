"""
Golden checks: per-login Role/Job grounding — no ambient skill packs.
...
class TestJdLexicon:
    ...
    def test_brim_role_prompt_has_materials_only(self):
        ...
        assert "BRIM" in lex or "BILLING" in lex
        ...
        low = user.lower()
        assert "brim" in low
        assert "interview materials" in low or "materials rule" in low
        assert "materials-only" in low or "materials only" in low or "role" in low
        # No hardcoded domain lock labels
        assert "stay strictly inside sap attp" not in low
        assert "active domain: sap attp" not in low

    def test_soft_empty_prompt_not_domain_locked(self):
        from answer_engine import _build_user_prompt, _fallback_strategy
        ...
        assert "interview materials" in low or "no role" in low or "materials" in low
...
class TestJdPromptConstruction:
    def test_user_prompt_includes_role_terms(self):
        ...
        assert "Role:" in user or "ROLE:" in user
        assert q[:40] in user
        assert "VOCABULARY + REASONING" not in user
        jar = " ".join(strategy.get("jargon_bank") or []).upper()
        assert any(t in jar for t in ("ATTP", "SAP", "AGGREGATION", "SHIPPING"))

    def test_ml_question_prompt_does_not_force_role(self):
        from answer_engine import _build_user_prompt, _fallback_strategy
        ...
        assert "sap attp techno-functional" not in low
        jar = " ".join(strategy.get("jargon_bank") or []).upper()
        # lexicon from question only — may include OVERFITTING
